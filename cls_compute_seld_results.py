...
class ComputeSELDResults(object):
    ...
    def get_nb_files(file_list, tag='all'):
        ...
        for _filename in file_list:
            ...
            if tag == 'all':
                ...
                _items = _filename.split("_")
                _ind = int(_items[1].replace("room", ""))
    ...
    def get_SELD_Results(self, pred_files_path, is_jackknife=False):
        ...
        if is_jackknife:
            ...
            for leave_file in tqdm(pred_files, leave=False, desc="Jackknife Evaluation Loop : "):
...
if __name__ == "__main__":
    pred_output_format_files = './results_audio/1_salsafoa_full_baseline_spatialscaper_round2_dev_split0_multiaccdoa_foa_20240721221014_test'  # Path of the DCASEoutput format files
    params = parameters.get_params("1", verbose=False)
    # Compute just the DCASE final results
    use_jackknife = False
    params['evaluate_distance'] = False
    eval_dist = params['evaluate_distance'] if 'evaluate_distance' in params else False
    score_obj = ComputeSELDResults(params, ref_files_folder='./DCASE2024_SELD_dataset/metadata_dev')
    if eval_dist:
