...
# Change working dir to file dir so relative paths work
abspath = os.path.abspath(__file__)
dname = os.path.dirname(abspath)
print("Changing directory to file directory --> {}".format(dname))
os.chdir(dname)
...
def compute_scaler(feature_dir: str, audio_format: str) -> None:
    ...
    with Progress() as scaler_progress:
        scaler_task = scaler_progress.add_task("Extracting Scalers", total=len(feature_fn_list))
        for count, feature_fn in enumerate(feature_fn_list):
            ...
            scaler_progress.advance(scaler_task)
...
def extract_features(data_dir: str = "../DCASE2024_SELD_dataset", feat_dir: str = "../DCASE2024_SELD_dataset/feat_labels",
                     audio_format: str = "foa", fs: int = 24000, n_fft: int = 512,
                     win_length:int = 512, hop_length: int = 300, fmin: int = 50, fmax: int = 12000,
                     n_mels: int = 128, feature_type: str = 'linspeciv',
                     task: str = 'feature_scaler', is_compressed_freq: bool = True,
                     transfer_data: bool = True) -> None:
    """
    Extract features
    ...
    if task in ['feature_scaler', 'feature']:
        for split in splits:
            ...
            feature_dir = os.path.join(feat_dir, feature_type, feature_description, split)
            print('\t\t Extracting from {} \n\t\t Placing features in {}'.format(audio_dir, feature_dir))
            ...
            with Progress() as progress:
                feature_task = progress.add_task("Feature Extraction", total = len(audio_fn_list))
                for count, audio_fn in enumerate(audio_fn_list):
                    ...
                    progress.console.print('{}, {}, {}'.format(count, audio_fn, audio_feature.shape))
    ...
    if task in ['feature_scaler', 'scaler']:
        feature_dir = os.path.join(feat_dir, feature_type, feature_description)
        compute_scaler(feature_dir=feature_dir, audio_format=audio_format)
        
    if transfer_data:
        dev_feat_dir = os.path.join(feature_dir, audio_format)
        with h5py.File(os.path.join(feature_dir, "{}_feature_scaler.h5".format(audio_format)), "r") as hf:
            mean = hf['mean']
            std = hf['std']
            
        final_folder = os.path.join(data_dir, "seld_feat_label_{}_dnorm_")
        
        for featfile in os.listdir(dev_feat_dir):
            if featfile.endswith('.h5'):
                hfile = h5py.File(os.path.join(dev_feat_dir, featfile), "r")
                feat_data = hfile['feature'][:]
...
if __name__ == '__main__':
    extract_features(feature_type='linspeciv')